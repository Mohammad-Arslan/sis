...
@section('content')
    ...
    {{-- <style>
        /* Style to make the select elements appear disabled */
        /* .dont-show {
    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="card">
                ...
                {{-- @dd($new_school_fee_structure) --}}
                <div class="card-body">
                    <form method="post" action="{{ route('fee_structure.update', $new_school_fee_structure->id) }}">
                        @csrf
                        <input name="parent_fee_structure" value="{{ $new_school_fee_structure->id }}"
                            type="hidden">
                        @method('PUT')
                        <div>
                            ...
                            <div class="form-section">
                                ...
                                <div class="row">
                                    <div class="col-md-4 col-sm-12">
                                        <div class="form-label-group in-border">
                                            <select class="filter load-select form-select dont-show" id="state_id"
                                                name="state_id" placeholder="state" data-target="city_id"
                                    ...
                                    <div class="col-md-4 col-sm-12">
                                        <div class="form-label-group in-border">
                                            <select
                                                class="filter form-select dont-show @if ($errors->has('city_id')) is-invalid @endif"
                                                id="city" name="city_id" aria-label="City select">
                                                <option value="">Please select</option>
                                    ...
                                    <div class="col-md-4 col-sm-12">
                                        <div class="form-label-group in-border">
                                            <select class="filter form-select dont-show" id="academic_year_id_new_school"
                                                name="academic_year_id_new_school">
                                                <option value="">Please select</option>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-4 col-sm-12">
                                        <div class="form-label-group in-border">
                                            <select class="filter form-select dont-show" name="class_group_id">
                                                <option value="">Please select</option>
                                ...
                                <div class="row">
                                    <div class="col-md-4 col-sm-12">
                                        <div class="form-label-group in-border">
                                            <input type="text" name="campus_area" class="form-control"
                        ...
                        <div class="row">
                            <div class="col-lg-12">
                                <div class="card">
                                    ...
                                    <div class="card-body">
                                        <div class="row">
                                            <input id="child_update" name="id" type="hidden">
                                            <input name="new_school_fee_structure_id"
                                                value="{{ $new_school_fee_structure->id }}" type="hidden">
                                            ...
                                            <div class="col-md-4 col-sm-12">
                                                <div class="form-label-group in-border">
                                                    <input type="date" name="approval_date" class="form-control"
                                                        id="approval_date" value="" required>
                                            ...
                                            <div class="col-md-4 col-sm-12">
                                                <div class="form-label-group in-border">
                                                    <input type="text" name="fee_details_remarks" class="form-control"
                                                        id="remarks" value="">
                                                    <label class="form-label">Remarks</label>
                                                </div>
                                            </div>
                                        ...
                                        <div style="display: flex; justify-content: flex-end; gap: 10px;">
                                            {{-- <a href="{{ route('fee_structure.index') }}" class="btn btn-danger">Back</a> --}}
                                            <button type="submit" class="btn btn-primary">Add / Update Fee Structure
                                                Options</button>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <tbody>
                                @foreach ($new_school_fee_structure->new_fee_structure_details as $record)
                                    <tr>
                                        ...
                                        <td>
                                            @permission(['fee-structure-status-approval'])
                                            <select data-id="{{ $record->id }}" name="fee_status_by_dd" class="fee_status_by_dd form-control">
                                                <option value="Pending"
                                            ...
                                            <input type="hidden" name="status_approval_date" id="status_approval_date"
                                                value="{{ $record->status_approval_date }}">
                                            @endpermission
                                        </td>
                                        <td>{{ $record->remarks }}</td>
                                        <td>
                                            <button data-id="{{ $record->id }}"
                                                class="btn btn-sm btn-primary edit-button"><i
...
@push('footer_scripts')
    ...
    {{-- <script>
        document.addEventListener('DOMContentLoaded', function() {
            const selects = document.querySelectorAll('.dont-show');
    ...
    <script>
        $(document).ready(function () {
            $('.fee_status_by_dd').on('change', function () {
                var selectedStatus = $(this).val();
                var statusApprovalDateField = $('#status_approval_date');

                if (selectedStatus === 'Approved' || selectedStatus === 'Cancelled') {
                    // Set the current date and time when status is Approved or Cancelled
                    var currentDate = new Date();
                    var formattedDate = currentDate.toISOString().slice(0, 10);
                    statusApprovalDateField.val(formattedDate);
                } else {
                    // Set status_approval_date to an empty value for other statuses
                    statusApprovalDateField.val('');
                }
            });
        });
    </script>
    ...
    <script>
        $(document).ready(function() {
            $('.edit-button').click(function() {
                ...
                var url = '/fee-structures/' + id + '/complete-edit';
                $.ajax({
                    url: url,
                    method: 'GET',
                    success: function(response) {
                        var data = response
                            .child_data;
                        console.log(data);
                        $('#child_update').val(data.id)
                        $('#approval_date').val(data.approval_date);
                        $('#nearest_bss_school').val(data.nearest_bss_school);
                        $('#fee_charges').val(data.fee_charges);
                        ...
                        $('#final_fee_charges').val(data.final_fee_charges);
                        $('#round_final_fee_charges').val(data.round_final_fee_charges);
