    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header align-items-center d-flex">
                <h4 class="card-title mb-0 flex-grow-1">Edit Fee Package</h4>
                <div class="flex-shrink-0">
                    <!-- <a href="{{ route('fee-packages.index') }}" class="btn btn-success btn-label btn-sm">
                            <i class="ri-check-double-line label-icon align-middle fs-16 me-2"></i> Add New Country
                        </a> -->
                    @permission('add-packages')
                        <a href="{{ route('fee-packages.create') }}" class="btn btn-sm btn-soft-success">
                            <i class="ri-add-circle-line align-middle me-1"></i> Add New Fee Package
                        </a>
                    @endpermission
                </div>
            </div><!-- end card header -->
            ...
            <div class="card-body">
                ...
                <div class="live-preview">
                    <form class="row g-3 needs-validation" novalidate action="{{ route('fee-packages.update', $feePackage->id) }}"
                        method="post">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="col-md-4 col-sm-12 mt-4">
                            <div class="form-label-group in-border">
                                <input type="text" class="form-control @if ($errors->has('package_name')) is-invalid @endif"
                                    ...
                                    value="{{ $feePackage->package_name }}" required>
                        ...
                        <div class="col-md-4 col-sm-12 mt-4">
                            <div class="form-label-group in-border">
                                <input type="text" class="form-control @if ($errors->has('abbreviation')) is-invalid @endif"
                                    ...
                                    value="{{ $feePackage->abbreviation }}" required>
                                    ...
                                    @if (isset($fee_package_types))
                                        @foreach ($fee_package_types as $fee_package_type)
                                            <option value="{{ $fee_package_type->id }}"
                                                {{ $fee_package_type->id == $feePackage->fee_package_type_id ? 'selected' : '' }}>
                                    ...
                                    @if (isset($companies))
                                        @foreach ($companies as $company)
                                            <option value="{{ $company->id }}"
                                                {{ $company->id == $feePackage->company_id ? 'selected' : '' }}>
                        ...
                        <div class="col-md-4 col-sm-12 mt-4">
                            <div class="form-label-group in-border">
                                <select class="load-select form-select @if ($errors->has('branch_id')) is-invalid @endif"
                                    ...
                                    <option value="">Please select</option>
                                    @if ($feePackage->branch_id !== null)
                                        @foreach ($branches as $branch)
                                            <option value="{{ $branch->id }}"
                                                {{ $branch->id == $feePackage->branch_id ? 'selected' : '' }}>
                        ...
                        <div class="col-md-4 col-sm-12 mt-4">
                            <div class="form-label-group in-border">
                                <select class="form-select @if ($errors->has('academic_year_id')) is-invalid @endif" id="branch"
                                    ...
                                    <option value="">Please select</option>
                                    @if ($feePackage->academic_year_id !== null)
                                        @foreach ($academic_years as $academic_year)
                                            <option value="{{ $academic_year->id }}"
                                                {{ $academic_year->id == $feePackage->academic_year_id ? 'selected' : '' }}>
                                                {{ $academic_year->title }}
                                            </option>
                                        @endforeach
                                    @endif
                                        ...
                                        <select
                                            ...
                                            @foreach ($classes as $class)
                                                <option value="{{ $class->id }}"
                                                    {{ $feePackage->from_class_id == $class->id ? 'selected' : '' }}>
                                ...
                                <div class="col-md-6 col-sm-12">
                                    <div class="form-label-group in-border">
                                        <select
                                            ...
                                            @foreach ($classes as $class)
                                                <option value="{{ $class->id }}"
                                                    {{ $feePackage->to_class_id == $class->id ? 'selected' : '' }}>
                                                    {{ $class->class_name }}</option>
                                            @endforeach
                                        </select>
                        ...
                        <div class="col-md-12 col-sm-12 mt-4">
                            <div class="form-label-group in-border">
                                <textarea class="form-control @if ($errors->has('description')) is-invalid @endif" name="description"
                                    id="feePackagesDescription" placeholder="Enter Fee Packages description here...">{{ $feePackage->description }}</textarea>
