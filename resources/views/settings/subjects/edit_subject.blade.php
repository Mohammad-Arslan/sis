    ...
    <div class="card">
        <div class="card-header align-items-center d-flex">
            ...
            <div class="flex-shrink-0">
                @permission('add-subject')
                    <a href="{{ route('subjects.index') }}" class="btn btn-success btn-label btn-sm">
                        <i class="ri-add-fill label-icon align-middle fs-16 me-2"></i> Add New Subject
                    </a>
                @endpermission
            </div>
        </div><!-- end card header -->
        ...
        <div class="card-body">
            <div class="live-preview">
                <form class="row g-3 needs-validation" novalidate action="{{ route('subjects.update', $subject->id) }}"
                    method="post">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="col-md-4 col-sm-12">
                        <div class="form-label-group in-border">
                            <input type="text" class="form-control" name="abbreviation" id="subjectAbbr"
                                placeholder="Please enter abbreviation" value="{{ $subject->abbreviation }}">
                    ...
                    <div class="col-md-4 col-sm-12">
                        <div class="form-label-group in-border">
                            <input type="text" class="form-control" name="sort_no" id="subjectAbbr"
                                placeholder="Please enter sort_no" value="{{ $subject->sort_no }}">
                            ...
                            <select class="form-select mb-3" id="languageName" name="language_id" required>
                                ...
                                @foreach ($languages as $language)
                                    <option value="{{ $language->id }}"
                                        @if ($subject->language_id == $language->id) {{ 'selected' }} @endif>
                                        {{ $language->language_name }}</option>
                                @endforeach
                            </select>
                            <label for="languageName" class="form-label">Languages</label>
                            <div class="invalid-tooltip">Select the Languages!</div>
                        </div>
                    </div>
                    <div class="col-md-6 col-sm-12">
                        <div class="form-label-group in-border">
                            <select class="form-select mb-3" id="subjectGroupName" name="subject_group_id">
                                ...
                                @foreach ($subjectGroups as $subjectGroup)
                                    <option value="{{ $subjectGroup->id }}"
                                        @if ($subject->subject_group_id == $subjectGroup->id) {{ 'selected' }} @endif>
                    ...
                    <div class="col-md-6 col-sm-12">
                        <div class="form-label-group in-border">
                            <select class="form-select mb-3" id="isAcademic" name="is_academic" required>
                                ...
                                <option value="1" @if ($subject->is_academic == '1') {{ 'selected' }} @endif>Yes
                                </option>
                                <option value="0" @if ($subject->is_academic == '0') {{ 'selected' }} @endif>No
                                </option>
                            </select>
                    ...
                    <div class="col-md-6 col-sm-12">
                        <div class="form-label-group in-border">
                            <select class="form-select mb-3" id="subject_type" name="subject_type" required>
                                ...
                                <option value="Major" @if ($subject->subject_type == 'Major') {{ 'selected' }} @endif>
                                    Major</option>
                                <option value="Minor" @if ($subject->subject_type == 'Minor') {{ 'selected' }} @endif>
                                    Minor</option>
                            </select>
