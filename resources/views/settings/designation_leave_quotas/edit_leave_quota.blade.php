<div class="col-lg-12">
    <div class="card">
        <div class="card-header align-items-center d-flex">
            <h4 class="card-title mb-0 flex-grow-1">Update Designation Quota</h4>
        </div><!-- end card header -->
        ...
        <div class="card-body">
            <div class="live-preview">
                <form class="row g-3 needs-validation" novalidate action="{{ route('designation-leave-quota.store') }}" method="post">
                        ...
                        <div class="form-label-group in-border">
                            <select class="form-control" id="designation_id" name="designation_id" required>
                                ...
                                @forelse(designations() as $designation)
                                    <option value="{{ $designation->id }}" {{ ($designation->id == $leaveQuota->designation_id) ? 'selected' : '' }}>{{ $designation->designation_name }}</option>
                                @empty
                                @endforelse
                            </select>
                        ...
                        <div class="form-label-group in-border">
                            <select class="form-control" id="leave_type_id" name="leave_type_id" required>
                                ...
                                @forelse(leaveTypes() as $leaveType)
                                    <option value="{{ $leaveType->id }}" {{ ($leaveType->id == $leaveQuota->leave_type_id) ? 'selected' : '' }}>{{ $leaveType->name }}</option>
                                @empty
                                @endforelse
                            </select>
                    ...
                    <div class="col-md-4 col-sm-4">
                        <div class="form-label-group in-border">
                            <input type="number" min="1" class="form-control @if($errors->has('no_of_allowed_leaves')) is-invalid @endif" id="no_of_allowed_leaves" name="no_of_allowed_leaves" placeholder="Please enter number of allowed leaves" value="{{ $leaveQuota->no_of_allowed_leaves }}" required>
