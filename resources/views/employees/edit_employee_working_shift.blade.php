<div class="col-lg-12">
    <div class="card">
        <div class="card-header align-items-center d-flex">
            <h4 class="card-title mb-0 flex-grow-1">Edit Employee Working Shift</h4>
            <div class="flex-shrink-0">
                @permission('add-employee-working-shift')
                <a href="{{ route('employee-shift.index') }}" class="btn btn-sm btn-soft-success">
                    <i class="ri-add-circle-line align-middle me-1"></i> Edit Employee Working Shift
                </a>
                @endpermission
            </div>
        </div><!-- end card header -->
        ...
        <div class="card-body">
            <div class="live-preview">
                <form class="row g-3 needs-validation" novalidate action="{{ route('employee-shift.update', $employeeWorkingDay->id) }}" method="post">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="form-label-group in-border">
                            <select class="form-select @if($errors->has('working_day_id')) is-invalid @endif" id="working_day_id" name="working_day_id" aria-label="Working Days" required>
                                ...
                                @foreach ($working_days as $working_day)
                                    <option value="{{ $working_day->id }}" {{ $employeeWorkingDay->working_day_id == $working_day->id ? 'selected' : '' }}>{{ $working_day->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-label-group in-border">
                            <select class="form-select @if($errors->has('working_shift_id')) is-invalid @endif" id="working_shift_id" name="working_shift_id" aria-label="Working Shifts" required>
                                ...
                                @else
                                    ...
                                    @endphp
                                @endif
                                    <option value="{{ $working_shift->id }}" {{ $employeeWorkingDay->working_shift_id == $working_shift->id ? 'selected' : '' }}>{{ $working_shift->start_time." - ".$working_shift->end_time. "  [".$Status."] " }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-12 text-end">
                        @if(isset($employee->id))
                            ...
                            <input type="hidden" name="id"  value="{{ $employeeWorkingDay->id }}" id="id">
