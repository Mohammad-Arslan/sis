    ...
    <div class="row">
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h1 class="h3 mb-2">Edit Asset</h1>
                    <p class="text-muted">Update asset information and details.</p>
                </div>
                <div>
            ...
            <form id="editAssetForm" enctype="multipart/form-data">
                ...
                <div class="form-section">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control" id="assetName" name="name" value="{{ $asset->name }}" required>
                            <div class="invalid-feedback" id="name-error"></div>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="assetTag" class="form-label">Asset Tag <span class="text-danger">*</span></label>
                            <div class="input-group">
                                <input type="text" class="form-control" id="assetTag" name="asset_tag" value="{{ $asset->asset_tag }}" required>
                ...
                <div class="form-section">
                    ...
                    <div class="row">
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="text" class="form-control" id="serialNumber" name="serial_number" value="{{ $asset->serial_number }}">
                ...
                <div class="form-section">
                    ...
                    <div class="row">
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="date" class="form-control" id="purchaseDate" name="purchase_date" value="{{ $asset->purchase_date ? $asset->purchase_date->format('Y-m-d') : '' }}">
                            <div class="invalid-feedback" id="purchase_date-error"></div>
                        </div>
                        <div class="col-md-4 mb-3">
                            <label for="purchasePrice" class="form-label">Purchase Price</label>
                            <input type="number" class="form-control" id="purchasePrice" name="purchase_price" step="0.01" min="0" value="{{ $asset->purchase_price }}">
                            <div class="invalid-feedback" id="purchase_price-error"></div>
                        </div>
                        <div class="col-md-4 mb-3">
                            <label for="warrantyEndDate" class="form-label">Warranty End Date</label>
                            <input type="date" class="form-control" id="warrantyEndDate" name="warranty_end_date" value="{{ $asset->warranty_end_date ? $asset->warranty_end_date->format('Y-m-d') : '' }}">
                ...
                <div class="form-section">
                        ...
                        <div class="col-md-3 mb-3">
                            ...
                            <select class="form-select" id="condition" name="condition" required>
                                ...
                                @foreach($conditions as $condition)
                                    <option value="{{ $condition['value'] }}" {{ $asset->condition == $condition['value'] ? 'selected' : '' }}>
                                        {{ $condition['label'] }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-3 mb-3">
                            ...
                            <select class="form-select" id="asset_status" name="status" required>
                                ...
                                @foreach($statuses as $status)
                                    <option value="{{ $status['value'] }}" {{ $asset->status == $status['value'] ? 'selected' : '' }}>
                                        {{ $status['label'] }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-3 mb-3">
                            ...
                            <select class="form-select" id="currentBranch" name="current_branch_id" required>
                                ...
                                @foreach($branches as $branch)
                                    <option value="{{ $branch->id }}" {{ $asset->current_branch_id == $branch->id ? 'selected' : '' }}>
                        ...
                        <div class="col-md-3 mb-3">
                            ...
                            <select class="form-select" id="currentDepartment" name="current_department_id">
                                ...
                                @foreach($departments as $department)
                                    <option value="{{ $department->id }}" {{ $asset->current_department_id == $department->id ? 'selected' : '' }}>
                    ...
                    <div class="row">
                            ...
                            <select class="form-select" id="assignedTo" name="assigned_to_user_id">
                                ...
                                @foreach($users as $user)
                                    <option value="{{ $user->id }}" 
                                            ...
                                            {{ $asset->assigned_to_user_id == $user->id ? 'selected' : '' }}>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <div class="image-upload-area" id="imageUploadArea">
                                ...
                                <input type="file" id="imageUpload" name="new_images[]" multiple accept="image/*" style="display: none;">
                            </div>
                    ...
                    <!-- Current Images -->
                    @php
                        // Always decode image_url if it's a string (JSON from DB)
                        ...
                        function assetImageSrc($path) {
                            // If path already starts with 'http' or 'https', return as is
                            if (preg_match('/^https?:\/\//', $path)) {
                                return $path;
                            }
                            // If path starts with '/storage/', remove leading slash for asset()
                            ...
                            <div class="col-12 mb-3">
                                ...
                                <div class="current-images-container">
                                    @foreach($imageUrls as $imagePath)
                                        <div class="current-image-item" data-image-path="{{ $imagePath }}">
                                            <img src="{{ assetImageSrc($imagePath) }}" alt="Current Image">
                                            <button type="button" class="remove-current-image" data-image-path="{{ $imagePath }}" title="Remove Image">
                ...
                <div class="form-section">
                    <div class="d-flex justify-content-end">
                        <a href="{{ route('fixed-assets.assets.index') }}" class="btn btn-secondary me-3">Cancel</a>
...
$(document).ready(function() {
    ...
    $('#editAssetForm').on('submit', function(e) {
        ...
        $.ajax({
            url: '{{ route("fixed-assets.assets.update", $asset->id) }}',
            method: 'POST',
            data: formData,
            processData: false,
            contentType: false,
...
function setupDynamicUserFiltering() {
    ...
    const currentUserId = userSelect.val(); // Store the currently selected user ID
                ...
                success: function(response) {
                    if (response.length === 0) {
                        ...
                        response.forEach(function(user) {
                            ...
                            const option = $(`<option value="${user.id}">${displayName}${departmentName}</option>`);
                            
                            // If this is the currently assigned user, mark it as selected
    ...
    // Call updateUsers on initial load if department or branch is selected
    if (departmentSelect.val() || branchSelect.val()) {
        updateUsers();
    }
}
